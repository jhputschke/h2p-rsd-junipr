...
*Status: **RUN (twice). Results: [`PROD_TEST_v0_RESULTS.md`](PROD_TEST_v0_RESULTS.md).*** Unlike the other
...
| 7 | [`PROD_TEST_v0_RESULTS.md`](PROD_TEST_v0_RESULTS.md) + `docs/figures/prod_test_v0/` | added during the run: the results belong in their own document, rewritten per re-run, not interleaved with the design. Figures regenerate with [`scripts/make_prod_test_figures.py`](../scripts/make_prod_test_figures.py) — `runs/` is gitignored, so a summary pointing there would render blank |
...
**The results live in [`PROD_TEST_v0_RESULTS.md`](PROD_TEST_v0_RESULTS.md)** — this file is
the design and the rationale, that one is what the run found. Kept apart deliberately: the
plan is written once and read for *why* a check exists, while the results are rewritten
every time the test is re-run, and interleaving them makes it impossible to tell which
paragraphs are commitments and which are observations.

Headline, so this file is not a dead end:

- **The arm passes.** Medoid/identity 0.902 and off-grid geo-median/identity 0.871, both
  below 1 and agreeing in sign.
- **Aux conditioning helps**: −0.0610 ± 0.0050 nat/jet, with the `nx == 0` control
  band-consistent and every aux-carrying stratum 2× the band outside it.
- **The run was performed twice.** The first pass exposed an encoder defect — `gru` and
  `lundnet` both read `collate`'s zero-padding, so training and single-jet inference sat in
  different regimes — that invalidated three of its own headline conclusions. Fixed behind
  `encoder.mask_padding`; the grid was retrained; the pre-fix pass is archived at
  `runs/prod_test_v0_pre_encoder_fix/`.

That defect is the strongest argument for this plan existing. No unit test, the parity
check, or a training curve could have caught it: every one of them lives on **one side** of
the train/inference boundary, and only an end-to-end assessment crosses it.
