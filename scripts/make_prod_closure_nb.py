"""Generate notebooks/lund_distribution_closure_prod_test_v0.ipynb from the v2 notebook.

The production test needs `lund_distribution_closure_v2.ipynb` pointed at the held-out
file with five constants changed, three of which are not obvious and one of which
...
two closure populations drifting apart (docs/PLAN_prod_test_v0.md §7). So the variant is
...
if the committed notebook does not match, so the two cannot diverge silently.

The five values are NOT baked in as literals either. They are read at runtime from
`prod_test_v0_metrics.json`, which `notebooks/prod_test_v0.ipynb` §9 writes — so the
frozen `tau` and the fitted `(temperature, tilt)` can never disagree with the §6 fit that
produced them.

    python scripts/make_prod_closure_nb.py            # write the notebook
    python scripts/make_prod_closure_nb.py --check    # verify it is up to date
"""
...
OUT = REPO / "notebooks" / "lund_distribution_closure_prod_test_v0.ipynb"

# The section-0 lines this variant replaces, and what they become. Each must match
...
def build() -> dict:
    ...
    cell["source"] = (PRELUDE + body.rstrip("\n") + "\n" + EPILOGUE).splitlines(keepends=True)
    ...
    header = {
        ...
        "source": (
            ...
            "\n"
            "Five settings differ from v2's defaults, and they are read from"
            " `prod_test_v0_metrics.json`\n"
            ...
            "| `LENGTH_TILT` | the same fit — a scalar temperature alone cannot correct"
            " this head (3.10× → 2.24×, vs 1.07× with the tilt) |\n"
            "\n"
            "Run [`notebooks/prod_test_v0.ipynb`](prod_test_v0.ipynb) first; this reads"
            " its artifact.\n"
